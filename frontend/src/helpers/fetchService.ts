...
export const fetchService = async (url: string, method: httpMethod = 'GET' , body?: any): Promise<any> => {
  ...
  const request = await fetch(url, requestConfig)
    .then(res => {
      if (!res.ok) {
        ...
        } else if ((res.status > 400 && res.status < 404) || (res.status >405 && res.status < 500)) {
